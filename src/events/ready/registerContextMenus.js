  ...
  try {
    ...
    for (const localContextMenu of localContextMenus) {
      ...
      if (existingContextMenu) {
        if (localContextMenus.deleted) {
          await applicationContextMenus.delete(existingContextMenu.id);
          console.log(
            `[INFO] Application command ${contextMenuName} has been deleted.`
              .red
          );
          continue;
        }
        ...
        if (localContextMenu.deleted) {
          console.log(
            `[INFO] Application command ${contextMenuName} has been skipped, since property "deleted" is set to "true".`
              .grey
          );
          continue;
        }
        ...
        console.log(
          `[INFO] Application command ${contextMenuName} has been registered.`
            .green
        );
      }
    }
